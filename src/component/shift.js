...
export default function Shift(props) {
  ...
  useEffect(() => {
    ...
    if (shiftData !== undefined && shiftData.shift !== null) {
      ...
      for (
        i = 0;
        i < (shiftData.shift.length < 5 ? shiftData.shift.length : 5);
        i++
      ) {
            ...
            <>
              <Paper className={Name} onClick={() => handleSeatOpened(idx)}>
                ...
                <Typography
                  variant="subtitle1"
                  className={classes.paperText}
                >{`${d_hour}:${d_minute}`}</Typography>
                <ArrowRightAltIcon
                  style={{ fontSize: "22px", margin: "10px 0 10px 0" }}
                />
                <Typography
                  variant="subtitle1"
                  className={classes.paperText}
                >{`${a_hour}:${a_minute}`}</Typography>
              </Paper>
                ...
                <Collapse in={seatOpened[idx]} timeout="auto" unmountOnExit>
                  <div className={classes.bigContainer}>
                    <IconButton
                      ...
                      onClick={() => handlePreviousClick(idx)}
                    >
                      ...
                      <div className={classes.carContainer}>
                        <div className={classes.car}>
                          {seats.slice(0, 40).map((val, idx) => {
                            const class_name = val.available
                              ? seatChosen[idx + 0]
                                ? classes.seatChosen
                                : classes.seat
                              : classes.seatDisable;
