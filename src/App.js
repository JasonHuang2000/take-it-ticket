...
import {
  ...
  UPDATE_SEAT_MUTATION,
  UPDATE_RECORD_MUTATION,
} from "./graphql/mutation.js";

const theme = createMuiTheme({
  typography: {
    fontFamily: ["Roboto", "Arial", "American Typewriter", "Arial Black"].join(
      ","
    ),
  },
});
...
export default function App() {
  ...
  const { loading: shiftLoading, data: shiftData } = useQuery(SHIFT_QUERY, {
    variables: {
  ...
  const handleEnterOption = () => {
    ...
    if (userData.user === null) {
      ...
      userData.user.userid === ID &&
      savedPwd === userData.user.password
    ) {
      setWrongID(false);
      setWrongPwd(false);
      setSignIn(true);
  ...
  return (
    <ThemeProvider theme={theme}>
      <Modal open={signInOpen} onClose={handleSignInClose}>
        <SignIn
      ...
      {!enterOption ? (
        ...
        <Record name={name} userData={userData} userLoading={userLoading} />
      )}
    </ThemeProvider>
  );
}
